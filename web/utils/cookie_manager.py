"""
Cookie Manager - Solves the problem of Streamlit session state loss on page refresh
"""
...
try:
    ...
    st.warning("⚠️ streamlit-cookies-manager not installed, Cookie functionality unavailable")

class CookieManager:
    """Cookie manager for persistent storage of analysis state"""

    def __init__(self):
        self.cookie_name = "tradingagents_analysis_state"
        self.max_age_days = 7  # Cookie validity period 7 days
        ...
        if COOKIES_AVAILABLE:
            try:
                ...
                if not self.cookies.ready():
                    # If not ready, show waiting message first, then stop execution
    ...
    def set_analysis_state(self, analysis_id: str, status: str = "running",
        ...
        try:
            ...
            # Store to session state (as backup)
    ...
    def get_analysis_state(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            # First try to get from session state (if exists)
