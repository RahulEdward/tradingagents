...
class ThreadTracker:
    ...
    def register_thread(self, analysis_id: str, thread: threading.Thread):
        ...
        with self._lock:
            ...
            logger.info(f"📊 [Thread Tracking] Register analysis thread: {analysis_id}")
    ...
    def unregister_thread(self, analysis_id: str):
        ...
        with self._lock:
            if analysis_id in self._threads:
                ...
                logger.info(f"📊 [Thread Tracking] Unregister analysis thread: {analysis_id}")
    ...
    def is_thread_alive(self, analysis_id: str) -> bool:
        ...
        with self._lock:
            ...
            if not is_alive:
                # Thread is dead, auto cleanup
                del self._threads[analysis_id]
                logger.info(f"📊 [Thread Tracking] Thread is dead, auto cleanup: {analysis_id}")
    ...
    def get_alive_threads(self) -> Dict[str, threading.Thread]:
        ...
        with self._lock:
            ...
            for analysis_id in dead_threads:
                del self._threads[analysis_id]
                logger.info(f"📊 [Thread Tracking] Clean up dead thread: {analysis_id}")
...
def get_analysis_thread_info(analysis_id: str) -> Optional[Dict]:
    """Get analysis thread information"""
...
def get_all_analysis_threads() -> Dict[str, Dict]:
    """Get all analysis thread information"""
...
def check_analysis_status(analysis_id: str) -> str:
    ...
    # Thread doesn't exist, check progress data to determine final status
    try:
